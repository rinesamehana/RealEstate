...
export default observer(function MenyraPagesesDetails() {
  const { menyraPagesesStore } = useStore();

  const {
    selectedMenyraPageses: menyrapageses,
    loadMenyrPagese,
    loadingInitial,
  } = menyraPagesesStore;
  ...
  useEffect(() => {
    if (menyraPagesesId) loadMenyrPagese(menyraPagesesId);
  }, [menyraPagesesId, loadMenyrPagese]);

  if (loadingInitial || !menyrapageses) return <LoadingComponent />;
  return (
    <div className="new">
      <Sidebar />
          ...
          <div className="newContainer">
            ...
            <Card fluid style={CardStyles}>
              <Card.Content style={CRStyles}>
                Pay Method: {menyrapageses.menyraePageses}
              </Card.Content>
              <Card.Content>
