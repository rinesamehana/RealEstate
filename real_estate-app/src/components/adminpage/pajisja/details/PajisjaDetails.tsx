...
export default observer(function PajisjaDetails() {
  const { pajisjaStore } = useStore();

  const {
    selectedPajisje: pajisja,
    loadPajisje,
    loadingInitial,
  } = pajisjaStore;
  ...
  useEffect(() => {
    if (pajisjaId) loadPajisje(pajisjaId);
  }, [pajisjaId, loadPajisje]);

  if (loadingInitial || !pajisja) return <LoadingComponent />;
  return (
    <div className="new">
      <Sidebar />
          ...
          <div className="newContainer">
            ...
            <Card fluid style={CardStyles}>
              <Card.Content style={CRStyles}>
                Equipment: {pajisja.llojiPajisjes}
              </Card.Content>
              <Card.Content>
